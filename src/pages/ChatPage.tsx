...
function ChatPage() {
  ...
  const [mode, setMode] = useState<'balanced' | 'fast' | 'creative'>(getUserSettings().preferredMode);
  ...
  useEffect(() => {
    if (!user) return;
    bootstrapUserChats(user.id, mode);
  ...
  const sendMessage = async (event: FormEvent) => {
    ...
    if (!content || !activeChat || !user || !session?.access_token || isSubmitting) return;
      ...
      const response = await completeChat(
        {
          message: content,
          mode,
          chat_history: activeChat.messages.map((message) => ({ role: message.sender, content: message.content })),
        },
        session.access_token,
      );

      const answer = response.response ?? response.answer ?? response.message ?? 'Sem resposta da IA.';
      let partial = '';
      for (const char of answer) {
        partial += char;
        await updateLastAssistantMessage(user.id, activeChat.id, partial, response.model ?? mode);
        await wait(10);
      }
    } catch (apiError) {
      setError(formatApiError(apiError));
      await updateLastAssistantMessage(user.id, activeChat.id, 'Desculpe, não consegui responder agora. Tente novamente.', mode);
    } finally {
      setTyping(false);
      setSubmitting(false);
    }
  ...
  return (
    <section className="flex h-[calc(100vh-140px)] flex-col">
      <div className="mb-3 flex items-center justify-between">
        ...
        <select
          value={mode}
          onChange={(event) => setMode(event.target.value as 'balanced' | 'fast' | 'creative')}
      ...
      <div className="mb-4 flex-1 space-y-4 overflow-y-auto rounded-2xl border border-slate-800 bg-slate-900/40 p-4">
        {activeChat?.messages.length === 0 && <p className="text-center text-sm text-slate-500">Faça sua primeira pergunta para a Zent IA.</p>}
